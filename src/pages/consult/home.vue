<template>
    <view class="chat-container">
      ...
      <!-- 固定在顶部的信息卡片区域 -->
      <view v-if="healthData.totalScore > 0" class="fixed-info-cards" ref="infoCardsRef">
        <!-- 折叠控制按钮 -->
      ...
      <div 
        class="chat-messages"
        ref="chatMessagesRef"
        :class="{ 'collapsed': isCollapsed, 'no-info-cards': healthData.totalScore <= 0 }"
      >
  ...
  const updateChatAreaPosition = () => {
    // 如果没有健康数据，不需要更新位置
    if (healthData.value.totalScore <= 0) {
      return;
    }
  ...
  const setUpData = async () => {
    ...
    chatMessages.value.push({
      type: 'user',
      content: t('consult.initialQuestion'),
      time: timeString,
      showTime: shouldShowTime(currentTime)
    });
    
    // 清空输入框
    const userMessage = t('consult.initialQuestion');
    userInput.value = '';
  ...
  onMounted(async () => {
    ...
    if (options.language) {
      ...
      if (options.language === 'zh' || options.language === 'zh-CN') {
        language = 'zh-Hans';
      }
      setLocale(language);
    }
    if (options.userId) {
      ...
      // 1. 检查 customer 是否存在 Check if customer exists
      if (!options.customer) {
        uni.showToast({
          title: '参数错误',
          icon: 'none',
      ...
      // 2. 检查 customer 值是否正确 Check if customer value is correct
      const expectedCustomer = 'lusHair2234db2f';
      if (options.customer !== expectedCustomer) {
        uni.showToast({
          title: '参数错误',
          icon: 'none',
          duration: 2000
      ...
      // 3. 设置对应的 key Set corresponding key
      const key = 'kFxLZ90Bz8Og';
      
      // 4. 检查 sign 是否存在 Check if sign exists
      if (!options.sign) {
        uni.showToast({
          title: '参数错误',
          icon: 'none',
      ...
      // 5. 验证 sign - 使用 phone + key + customer 顺序进行 MD5 加密
      // Validate sign - Use phone + key + customer order for MD5 encryption
      const signString = `${options.phone}${key}${options.customer}`;
      const calculatedSign = md5(signString);
      
      // 不区分大小写进行比较 Case-insensitive comparison
      if (options.sign.toLowerCase() !== calculatedSign.toLowerCase()) {
        console.error('签名验证失败 Sign validation failed:', {
          expected: calculatedSign,
          received: options.sign,
      ...
      // 签名验证通过，继续获取用户信息 Sign validated, proceed to get user info
      try {
        const userInfoResponse = await post('user/info', {
            phone: options.phone
        }) as UserResponse; // 添加类型断言 Add type assertion
        
        // 更新全局用户信息 Update global user info
        Object.assign(userStore.userInfo, userInfoResponse, {
            phone: options.phone // 确保手机号被保存 Ensure phone number is saved
        });
        ...
        // 如果是 code 500 错误，提示用户信息不存在 If code is 500, show user info does not exist
        if (error?.code === 500) {
          uni.showToast({
            title: '用户信息不存在',
            icon: 'none',
          ...
          // 其他错误显示通用提示 Other errors show generic message
          uni.showToast({
            title: error?.message || error?.msg || '获取用户信息失败',
            icon: 'none',
            duration: 2000
          });
        }
  ...
  .chat-messages {
    ...
    width: 100%;
    box-sizing: border-box;
    margin-top: calc(var(--info-cards-height, 220px));
    height: calc(100vh - var(--info-cards-height, 220px) - 68px);
    overflow-y: auto;
    background: #f8f8f8;
  }
  ...
  .chat-messages.collapsed {
    margin-top: calc(var(--info-cards-collapsed-height, 80px));
    height: calc(100vh - var(--info-cards-collapsed-height, 80px) - 68px);
  }
  
  /* 没有信息卡片时的高度调整 */
  .chat-messages.no-info-cards {
    margin-top: 0px;
